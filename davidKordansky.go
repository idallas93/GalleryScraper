...
func updateDavidKordansky(url string) {
	c := colly.NewCollector()
	var davidKordanskyExhibitions []Exhibition
	c.OnHTML("#exhibitions-container", func(e *colly.HTMLElement) {
		e.ForEach("a", func(i int, colElement *colly.HTMLElement) {
			...
			location := colElement.ChildText("p")
			davidKordanskyExhibitions = append(davidKordanskyExhibitions, Exhibition{
	...
	SaveToExcel(davidKordanskyExhibitions)
}
